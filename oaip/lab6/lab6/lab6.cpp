...
void insert(list*& p, int value)
{
    ...
    if (newP != NULL)
    {
        newP->number = value; // доступ к значению числового поля текущего объекта
        newP->next = p; // доступ к указателю на следующий объект в последовательности.
        p = newP;
    }
    else
